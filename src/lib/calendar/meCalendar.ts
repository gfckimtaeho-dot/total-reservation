...
// 각 셀은 그 날 본인 예약 목록(events)을 담아, 달력에서 바로 일정을 본다.
...
export async function loadMeCalendarMonth(
  ...
  const [reservations, businessHours, closures] = await Promise.all([
