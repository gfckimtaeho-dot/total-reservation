// v1 — Mint Calm (mainpage 컨셉 #1)
// 흰 사이드바 + 흰 메인 + bg-band 페일 라임 hero strip + 진행중 = 라임 채움.
// mainpage.png 톤 충실. 차분 + 산뜻.
    ...
    <div className="flex min-h-screen bg-zinc-50">
      <aside className="hidden w-60 shrink-0 border-r border-zinc-200 bg-white lg:flex lg:flex-col">
        ...
        <nav className="flex-1 px-3 py-4">
          <a className="flex items-center rounded-md bg-band/60 px-3 py-2 text-sm font-medium text-ink">
            대시보드
          </a>
          {NAV_ITEMS.map((n) => (
            <a key={n.key} className="flex items-center rounded-md px-3 py-2 text-sm text-zinc-700 transition hover:bg-zinc-50 hover:text-ink">
              {n.label}
            </a>
          ))}
        </nav>
        <div className="border-t border-zinc-100 px-3 py-4">
          <a className="flex items-center rounded-md px-3 py-2 text-sm text-zinc-700 hover:bg-zinc-50 hover:text-ink">설정</a>
          <a className="flex items-center rounded-md px-3 py-2 text-sm text-zinc-700 hover:bg-zinc-50 hover:text-ink">로그아웃</a>
        </div>
      </aside>

      <main className="flex-1 overflow-x-hidden">
        {/* Hero band — mainpage 시그니처 */}
        <header className="bg-band">
          <div className="px-8 py-7">
            ...
            <h1 className="font-heading mt-2 text-3xl tracking-tight text-ink">
              <span className="italic">환영합니다,</span> {MOCK_BUSINESS.name}
            </h1>
          </div>
        </header>
        ...
        <div className="grid grid-cols-12 gap-4 p-6">
          ...
          {/* Membership expiring (full width) */}
          <section className="col-span-12 rounded-2xl border border-zinc-200 bg-white p-6">
            <SectionHead eyebrow="MEMBERSHIP · 7일 내 만료" title="갱신 권유 대상" />
            <ul className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              {MOCK_EXPIRING.map((m) => (
                <li
                  key={m.name}
                  className="flex items-center justify-between rounded-lg border border-zinc-100 bg-band/30 px-4 py-3"
                >
      ...
      {MOCK_MONTH.map((d) => {
        ...
        return (
          ...
          >
            ...
            {d.total > 0 && (
              <div className={`mt-0.5 text-[11px] tabular-nums ${SHADE_TEXT[lvl]}`}>
                {d.total}
              </div>
            )}
          </div>
