...
export function PackageTrainerCard({
  ...
  if (!assignedStaff) {
    return (
      <div className="mt-3 rounded-lg border border-amber-300/20 bg-amber-300/5 p-2.5 text-[11px] text-amber-200/90">
        {t("packageNoTrainer")}
      </div>
    );
  }
  ...
  return (
    <div className="mt-3 rounded-lg border border-white/5 bg-zinc-900/40">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
