...
import {
  previewScheduleDeletionImpact,
  applyScheduleDeletion,
  type ScheduleDeletionImpact,
} from "./schedule-actions";

// 헤더 backdrop-blur 가 만든 containing block 을 피하기 위해 portal 필수.
// (gotcha_dialog_portal_for_backdrop_blur 룰)
// 다이얼로그 자체는 다크 톤 고정 — 부모 화면 톤과 분리해 강한 알림 효과.
...
export function ScheduleDeleteDialog({
  ...
  const t = useTranslations("services.schedule.deleteDialog");
  const [loading, setLoading] = useState(true);
  const [impact, setImpact] = useState<ScheduleDeletionImpact | null>(null);
  const [applying, startApply] = useTransition();
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    let cancelled = false;
    void previewScheduleDeletionImpact(slug, scheduleId).then((r) => {
      if (cancelled) return;
      setImpact(r);
      setLoading(false);
    });
